...
int test_connack_message()
{
    ...
    mini_printf("test_connack_message\n");
    int ret;

    ret = connack_message_build(sp, ack_code, message, &message_len);
    if (ret != MQTT_SUCCESS) {
        mini_printf("connack_message_build failed\n");
        return -1;
    }
    mini_printf("connack_message_build success\n");
    ...
    if (ret != MQTT_SUCCESS) {
        mini_printf("connack_message_parse failed\n");
        return -1;
    }
    mini_printf("connack_message_parse success\n");
    mini_printf("ack code: %u", ack_code_get);
    mini_printf("sp: %u", sp_get);
